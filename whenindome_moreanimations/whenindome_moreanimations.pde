...
boolean resetter = true; // Option to turn off the background resetting function
int resetSpeed = 10;  // How often (in frames) to run the resetting function

// Stars are the background animation
Cluster[] clusters;
int numberOfClusters = 70;
int numberOfClusterstoDisplay = 70; // When there's lot of action happening, this number goes down
int action; // Counts up how much depth stuff is going on
float minStarDistance = -100;
float maxStarDistance = 100;
float minStarHue = -50;
float maxStarHue = 50;
float fadeInSpeed = 0.5;
...
boolean runWalkers = false;
boolean runSegments = false;
boolean runTriangles = true;
...
void setup() {
  ...
  theHues = new float[numberOfCircles];
  
  walkers = new Walker[numberOfWalkers];
  segments = new Segment[numberOfSegments];
  
  triangles = new Triangle[33];
  ...
  for (int i=0; i<lines.length; i++){
    lines[i] = new Line(-100*i, -100*i, random(0,360), 100, int(random(0,2)));
  }
  for (int i=0; i<circles.length; i++){
    circles[i] = new Circle(random(0,360), (width+500)/numberOfCircles*i);
    theHues[i] = random(hueCentral, (hueCentral+2)%360);
  }
  for (int i=0; i<walkers.length; i++){
    walkers[i] = new Walker();
  }
    for (int i=0; i<segments.length; i++){
    segments[i] = new Segment((360/numberOfSegments)*i);
  }
...
void draw() {
  ...
  //image(kinect.getDepthImage(), 620, 0); // useful for testing
  ...
  for (int x=0; x < depth.width; x++){
    for (int y=0; y < depth.height; y++){
      ...
      if (hue(currentColor) !=0){
        originalDepths[loc] = lerp(originalDepths[loc], hue(currentColor), 0.02);  
      }
    }
  }      
...
void clusterAnimation(){
  ...
  for (int i=0; i< numberOfClusterstoDisplay; i++){
    clusters[i].update();
    clusters[i].display();
  }  
...
void mapLeds(){
  ...
  opc.isoShortFirstLeft(1, 1, 0, 330, 1060, 90);        // 1A
  opc.isoShortFirstLeft(1, 2, 128, 330, 1060, 162);      // 1B
  opc.isoShortFirstRight(1, 3, 256, 330, 1060, 234);     // 1C
  opc.isoLongFirstLeft(7, 3, 3456, 330, 1060, 306);    // 7C
  opc.isoLongFirstRight(7, 2, 3328, 330, 1060, 18);    // 7B
  
  // top left pentagon
  opc.isoLongFirstRight(8, 3, 3968, 520, 390, 90);      // 8C
  opc.isoLongFirstLeft(2,2, 768, 520, 390, 162);       // 2B 
  opc.isoLongFirstRight(2,3,896, 520, 390, 234);      // 2C
  opc.isoLongFirstLeft(3,1,1024, 520, 390,306);     // 3A
  opc.isoLongFirstRight(9,1,4096, 520, 390, 18);     // 9A  
  
  // top right pentagon
  opc.isoLongFirstLeft(10, 1, 4608, width-520, 390, 90);      // 10A
  opc.isoLongFirstLeft(9, 3, 4480, width-520, 390, 162);      // 9C 
  opc.isoLongFirstRight(3,3,1408, width-520, 390, 234);     // 3C
  opc.isoLongFirstLeftWrong(4,1,1536, width-520, 390, 306);     // 4A
  opc.isoLongFirstRight(4,2,1664, width-520, 390, 18);     // 4B    
  
  // bottom right pentagon
  opc.isoShortFirstRight(5,3,2304, width-330, 1060, 90);   // 5C
  opc.isoLongFirstLeft(6,2,2816, width-330, 1060, 162);   // 6B
  opc.isoLongFirstRight(6,3,2944, width-330, 1060, 234);  // 6C
  opc.isoShortFirstLeft(5,1,2048, width-330, 1060, 306);  // 5A
  opc.isoShortFirstLeft(5,2,2176, width-330, 1060, 18);  // 5B 
  
  // centre pentagon
  opc.isoShortFirstRight(11,2,5248, width/2, 878, 126);   // 11B
  opc.isoLongFirstLeft(8,1,3584, width/2, 878, 198);   // 8A
  opc.isoShortFirstLeft(11,3,5376, width/2, 878, 270);  // 11C
  opc.isoLongFirstRight(10,3,4992, width/2, 878, 342);  // 10C
  opc.isoShortFirstLeft(11,1,5120, width/2, 878, 54);  // 11A  
  ...
  // top pair
  opc.ledEquilateral(3,2,1152, width/2, 125, 150); // 3B 
  opc.ledEquilateral(9,2,4224, width/2, 513, 330);  // 9B  
  
  // right pair
  opc.ledEquilateral(4,3,1792, 1595, 641, 342); // 4C 
  opc.ledEquilateral(10,2,4736, 1225, 759, 282);  // 10B    
  
  // left pair
  opc.ledEquilateral(2,1,512, width-1595, 641, 318); // 2A
  opc.ledEquilateral(8,2,3712, width-1225, 759, 18);  // 8B   
  
  // bottom pair (left first)
  opc.ledEquilateral(7,1,3072, 685, 1182, 305); // 7A
  opc.ledEquilateral(6,1,2560, width-685, 1182, 355);  // 6A   
  }
