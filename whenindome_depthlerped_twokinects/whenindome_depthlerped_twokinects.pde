...
float minStarDistance = -80;
float maxStarDistance = 80;
float minStarHue = -50;
float maxStarHue = 50;
float fadeInSpeed = 0.5;
float fadeOutSpeed = 20;
boolean clusterAnim= true;
...
void draw() {
  ...
  depth = multiKinect.get(0).getDepthImage();
    ...
    for (int x=0; x < depth.width; x++){
      for (int y=0; y < depth.height; y++){
        ...
        if (hue(currentColor) !=0){
          originalDepths[loc] = lerp(originalDepths[loc], hue(currentColor), 0.05);  
        }
      }
    }      
 // }
...
void clusterAnimation(){
   numberOfClusterstoDisplay = int( map(action, 0,1000, numberOfClusters, 0)); // map the amount of stars to the action, inversely
...
void oscEvent(OscMessage theOscMessage) {
  ...
  if (theOscMessage.addrPattern().equals("/resetnow")){
    ...
    if (theOscMessage.addrPattern().equals("/noClusters")){    
     clusterAnim = false;
  }
  if (theOscMessage.addrPattern().equals("/Clusters")){    
     clusterAnim = true;
  }
}
